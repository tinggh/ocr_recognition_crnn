...
def denseNet(**kwargs):
    model = DenseNet(num_in=1, num_init_features=64, growth_rate=8, block_config=(8, 8, 8, 8),
                     **kwargs)
    return model

def DenseNet18_128(**kwargs):
...
def ResNet18(**kwargs):
    model = ResNet(1, block=BasicBlock, layers=[2, 2, 2, 2], **kwargs)
    return model
